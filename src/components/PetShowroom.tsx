...
import { samplePets } from "@/seedData";
...
const PetShowroom = () => {
  ...
  const seedPets = async () => {
    try {
      const { data: existingPets, error: fetchError } = await supabase.from("pets").select("*");
      if (fetchError) {
        console.log("Using local sample pets (Supabase fetch failed)");
        return samplePets;
      }
      ...
      console.log("Using local sample pets");
      return samplePets;
    } catch (error) {
      console.log("Using local sample pets");
      return samplePets;
  ...
  useEffect(() => {
    const fetchAndSeedPets = async () => {
      try {
        const data = await seedPets();
        setPets(data);
      } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-10 relative z-10">
        ...
        {pets.length === 0 ? (
          <div className="flex flex-col items-center justify-center min-h-[400px] text-center">
            ...
            <p className="text-[var(--foreground)]/50 text-sm tracking-wide max-w-md">
              Our curated collection is being prepared. Check back soon for premium pets.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {pets.map((pet, idx) => {
              const isExclusive = pet.price > 50000 || pet.discount > 10;
              const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(`Hi Anu Pet Shop, I am interested in the ${pet.breed} ${pet.name}. Is it available at your Nagpur store?`)}`;
                ...
                >
                  <div className="relative aspect-[4/5] rounded-[40px] overflow-hidden border border-[var(--accent)]/10 glass-dark group-hover:border-[var(--accent)]/40 transition-all duration-700 shadow-2xl">
                    ...
                    <div className="absolute top-6 left-6 z-10">
                      <span className={`px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-[0.4em] ${
                        isExclusive 
                          ? 'bg-gradient-to-r from-[var(--accent)] to-amber-500 text-black' 
                          : 'bg-gradient-to-r from-green-600 to-emerald-500 text-white'
                      }`}>
                        {isExclusive ? 'EXCLUSIVE' : 'AVAILABLE'}
                      </span>
                    </div>

                    <img
                      src={pet.image_url || "https://via.placeholder.com/400x500?text=No+Image"}
                      alt={pet.name}
        ...
        >
          <p className="text-[var(--accent)]/20 text-[10px] tracking-[0.8em] uppercase font-bold">
            Curating Luxury Pets Since 2026
          </p>
        </motion.div>
      </div>
    </section>
